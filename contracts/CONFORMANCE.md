...
| §2.2 Randomness | `randomness/DrandSource` + `randomness/drand/DrandVerifier`, `lib/RandomLib` | built + live | drand evmnet beacon verified on chain (BN254, ~253k gas), request bound to a future round, permissionless `fulfill`, timed or operator-selected round per consumer, `isPending` false once the beacon may be public; see `DRAND-SPEC.md` for the trust model. Live on testnet since 2026-09-13 18:33 UTC: verifier `0x7231…cd72`, source `0xF43C…5f08`; vault, payroll and gacha switched by council; first mint (selected mode) and first spin (timed mode) settled through drand the same hour. Domain-separated rejection sampling unchanged. The operator commit-reveal (`OperatorCommitReveal`) and the Pyth Entropy adapter are legacy: every live consumer points at the drand source since 2026-09-13; the keeper keeps the reveal duty only for the retired r4 payroll's last New York days (through 2026-09-19), then drops it (DRAND-SPEC.md, "Retiring the operator source"). `refundUncommittedRequest` is refused once the source holds or may hold a word. |

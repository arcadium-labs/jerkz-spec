# Arcadium Labs : JERKZ
Work2Earn · Product and engineering specification · v0.21
...
- **Hybrid fees:** Mint costs 1 USDC per NFT; Burn costs 1 USDC per NFT. Each additional Burn → Mint reroll costs 2 USDC. Both fees fund the same Payroll Treasury after CRCL conversion. Gas and applicable randomness charges are separate. No batch discount or principal deduction; a quantity-q action costs q USDC in either direction.
...
4. **Complete wrap:** Transfer the assigned existing NFT to the permitted recipient and set ACTIVE. Convert pending principal into active NFT backing; earn the 1-USDC Mint fee for later CRCL conversion. Ownership, lifecycle, accounting and metadata changes are atomic. Failed delivery reverts that attempt and retains the same ASSIGNED character in the vault; the beneficiary may retry or change the permitted recipient. No new seed, generation or fee.
5. **Unwrap:** Current owner or authorized operator quotes the ordered ACTIVE IDs, expected generations, recipient, fee payer, quantity × 1 USDC, maxFee, deadline and configHash. Collect the exact USDC Burn fee from the caller or an explicitly authorized payer, separately from principal. Checkpoint VC credits, transfer NFTs to HybridVault, clear approvals, retire their characters and XP, switch to inactive metadata and emit ERC-4906. Return exactly r per NFT and stage the IDs for a later inventory batch. Earn the Burn fee only if the entire transaction succeeds; otherwise revert the fee, custody, retirement and principal transfer together. No partial batch or second charge on replay. No title quota is released because none exists. No price feed, randomness, image-service or CRCL-converter dependency. Active quest or third-party custody cannot be bypassed. Ordinary transfers, quest withdrawals and Payroll claims incur no hybrid fee; an eligible uncommitted Mint refund incurs no Burn fee.
...
Quotes must show the separate 1-USDC Burn fee, the full 100,000-JERKZ return, permanent loss of art, rarity, career, experience points (XP), health and items, plus surviving CRCL claims. Unwrap may escape illness or a bad career, but rewrap has no rare-title guarantee or item refund. “Lost forever” means no live-state restoration; blockchain history, cached metadata, published images and immutable files can remain.
...
The first Mint costs 1 USDC; each additional Burn → Mint reroll costs 2 USDC. A Mint followed by eventual Burn also costs 2 USDC, excluding gas and applicable randomness charges. Splitting the fee lowers initial entry cost; it does not increase the repeat-reroll cost. Initial Mints fund Payroll with 1 USDC rather than 2; the other 1 USDC is earned only on a completed Burn. Never budget an anticipated Burn fee. Fees discourage grinding but do not prove unprofitability. Test independent tier odds and premiums, higher-tier Payroll, avoided downtime and item costs, re-hiring and executable conversion costs. Wallet limits alone are insufficient. O1’s reference opening FDV is about USD 4,000: marginal 100,000 / 1B tokens ≈ USD 0.40 before impact; a 1-USDC Mint is 2.5× and a 2-USDC reroll is 5× that reference value. This is not a JERKZ price forecast; validate the actual Arc quote. [O1 opening liquidity](https://docs.o1.exchange/launchpad/trading/liquidity)
...
Mint and Burn each charge a fixed 1-USDC fee per NFT. Wrap uses verified asynchronous randomness, random inventory-ID selection, frozen batches and percentage-weighted titles. The user cannot choose the NFT ID or character. Unwrap retires the generation while preserving protected Payroll records. Reusable pre-generated IPFS art and dynamic JSON support jobs and approved cosmetic variants; no runtime artwork generation.
...
- **Hybrid fees:** Earn 1 USDC per Mint only after successful ACTIVE delivery and 1 USDC per Burn only after atomic redemption. Pending Mint fees remain protected; failed delivery retries add no fee. Record distinct MINT and BURN receipts once, retain earned USDC in the fee bucket and convert both into CRCL for the same Payroll Treasury. Converter failure cannot block a paid Burn. Never spend or deduct hybrid principal, pending payments or owed Payroll. Metadata updates, ordinary transfers, quest withdrawals and Payroll claims add no hybrid fee; eligible uncommitted Mint refunds return the unearned fee without a Burn fee.
...
- **Conversion:** **Mint / Burn** quotes from §2; balances, current inventory, permanent reset loss, retained claims, 1-USDC Mint fee and 1-USDC Burn fee per NFT. Show both fees before entry, the 2-USDC reroll total, separate gas and the full principal returned on Burn. Select wallet artwork to Burn; the confirmation explicitly states that experience points (XP) are permanently retired. Show request, randomness, assignment and delivery separately; retries retain the assignment.
...
**Measured impact:** Track activated wallets, completed 1-USDC Mints and 1-USDC Burns, Gacha USDC sales, depositor USDC claims, net inbound CCTP funding, Gateway-funded purchases, CRCL paid and returned users. Separate organic player payments from founder/team purchases, grant receipts, faucet tests and incentives. Count one economic purchase once even if it has a bridge, payment and proceeds distribution. Report gas separately; clock-ins, CRCL awards and ERC-20 item burns are not USDC payment volume. Hybrid Burn fees count separately from the returned JERKZ principal. Deduplicate users across the two products when using an authenticated first-party account; otherwise label the metric unique wallets, not people. §13.4 defines the measurement contract.
...
- **HybridVault:** Same-chain principal, inactive/reserved inventory, wrap requests, immutable assignments, ACTIVE delivery, unwrap and separate per-NFT Mint/Burn fee accounting. Never mints or burns either asset.
...
- **FeeLot:** Source contract and type (including MINT / BURN), unique receipt, input asset and amount, measured stock output, income day and recycled attribution.
...
- **previewConvert(quantity, direction):** Return Mint / Burn labels, fixed principal, per-NFT and total USDC fees, maxFee, deadline, configHash, versions and applicable draw odds; Burn (unwrap) quotes also bind IDs/generations and show losses and retained claims.
...
- **redeemNFTs(ordered tokenIds, expectedGenerations, recipient, maxFee, deadline, configHash):** Burn action (technical unwrap); validate the quote and authorized payer, collect exactly 1 USDC per NFT separately, checkpoint VC, retire current characters including XP, transfer existing NFTs into inventory and return exactly 100,000 JERKZ per NFT atomically. Earn and record the Burn fee once; any failure reverts the whole batch. Reject duplicate IDs, stale generations, expired/mismatched quotes and incorrect payment. A relayed payer requires explicit payment authorization; NFT operator approval alone cannot spend the owner’s USDC.
...
- **Conversion:** ConversionRequested, IdentityAssigned, CharacterGenerated, CharacterRetired, NFTRedeemed. Hybrid fee receipts identify MINT or BURN, payer, NFT quantity, USDC amount and a unique conversion/request reference; emit only for completed fee-earning actions.
...
- **Custody and principal:** Bootstrap creates exactly 10,000 IDs and permanently closes minting; no NFT burn path. Round-trip swaps preserve both asset supplies; separate item burns reduce JERKZ supply. Conversion, donations, pending requests, exhausted inventory and burns preserve reserves. Every ownership path clears stale approvals. Test exact decimals and 1-USDC-per-NFT fees in each direction, single/batch totals, full principal return, wrong/insufficient payment, payer authorization, expired/mismatched quotes, duplicate IDs and stale generations. Reverted Burns must leave fees, ownership, XP, principal and claims unchanged; successful Burns earn the fee once. Test no extra charge for Mint delivery retry, uncommitted Mint refund, ordinary transfer, quest withdrawal or Payroll claim; reject replay. Test prepared versus orphan custody and Burn availability during randomness, metadata or converter outages.

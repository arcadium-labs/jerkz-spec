# Arcadium Labs — JERKZ
Work2Earn · Product and engineering specification · v0.19

## Start here: JERKZ in plain English

JERKZ is an office-life game built around collectible NFT characters. You give your Jerk a job, choose how long they work and manage the consequences. Pay comes in **CRCL, tokenized Circle stock**.
...
1. **Get a Jerk.** Buy an existing NFT, or lock **100,000 JERKZ** and pay **2 USDC** to draw a new character. There are 10,000 permanent NFT IDs. Each wrap draws a Birth Title from percentage weights with no tier supply caps, independently draws one of four expressions at 25% each, then selects a compatible pre-generated IPFS option. The initial base-art/metadata catalog target is about 100,000 reusable options.
2. **Get hired.** Interns through Directors apply for jobs. Higher ranks have shorter waits and better hiring odds. C-suite, CEOs and Board Members get instant appointments and cannot be fired. Paid hiring items or an eligible executive referral can skip the lower ranks’ hiring process.
3. **Clock in anytime after hiring.** Choose an **8-, 16-, 24- or 48-hour shift** for the next funded hourly start. There is no daily enrollment window or second hiring process. The hourly grid starts at :30 each hour, anchored to the 9:30 a.m. New York workday reset. Your NFT locks when you accept the quote and unlocks at the stated shift end. The 24/48 options advance one/two local calendar days from that start; the quote shows the exact duration, including daylight-saving changes. Daily work limits, health and weekend rules still apply.
4. **Claim after your shift.** Results are **No Pay, Minimum Wage, Bonus or Jackpot**. Minimum Wage means a small variable game payout, not a legal wage or guaranteed amount. Funded pay is claimable when the full accepted shift has ended and its result has settled. There is no Friday payday, weekly wait, claim fee or early-pay discount. A late result can delay a claim but cannot extend NFT custody. Title, duration and performance affect your share of a fixed funded budget; they create no new money.
...
6. **Customize or start again.** Spend JERKZ on haircuts, clothes, glasses, watches and other items. Every payment permanently burns those tokens. A spouse perk can reduce stress gains. Unwrapping returns **100,000 JERKZ**, but permanently discards that character’s art, title, progress and paid items. Wrapping again creates a fresh character. Selling an existing NFT preserves its character; previously earned pay stays with the original beneficiary.

### What happens in a hybrid swap

The NFT is a reusable character slot. **Unwrap:** return it to escrow, clear its character and receive 100,000 JERKZ. **Wrap:** deposit 100,000 JERKZ, pay 2 USDC and draw a fresh character on an available NFT ID. You need not receive the ID you returned. Swaps transfer existing assets; they mint or burn neither asset. ERC-4906 tells supported marketplaces to reload the new metadata. This bidirectional hybrid token wrapper uses the escrow lifecycle in §2; refresh timing depends on the marketplace.
...
- **Generation:** One character’s life, from a fresh wrap until unwrap. The NFT ID can be reused; that character cannot.
...
Use **Payroll**, **Payroll Treasury**, **Job Board**, **Clock In**, **HR**, **On Thin Ice** (one strike), **Pink Slip**, **No Pay**, **Minimum Wage**, **Bonus** and **Jackpot**. These labels must expose the exact probability, fee and restriction. Jobs, moods and earnings are game states, not real employment or guaranteed income.
...
**Birth:** Fresh art, Birth Title and expression profile; initialize current Job Title = Birth Title, careerStreak = 0 and promotionProgress = 0, with a committed progression configuration; expression sets initial stress (§5). Zero XP, strikes, firings, fatigue, leave, crash, applications, weekend passes, wardrobe and relationship. Intern–Director start unemployed; Founder self-employed; C-suite, CEO and Board receive standing appointments; VC starts next-boundary eligibility on ACTIVE delivery.
...
Preserve generation and ownership counters, permanent token-ID daily Payroll use and sponsor quotas, frozen entries, funded VC intervals, original beneficiaries, reserved CRCL and paid flags. Old callbacks settle only their retired records and financial obligations. Generation-specific application limits and career backlog reset; permanent financial and replay limits do not. No dual worker / VC entitlement or second daily entry after reroll.
...
| Birth / current title | Birth draw weight | Current pay factor | Warm-up / hire chance | Strike per mistake |
|---|---|---|---|---|
| Intern | 40% | 1.00× | 8h / 60% | 100% |
| Junior / Associate | 23% | 1.10× | 4h / 75% | 80% |
| Specialist | 15% | 1.25× | 2h / 85% | 60% |
| Middle Manager | 10% | 1.50× | 1h / 95% | 40% |
| Director | 5% | 1.75× | 30m / 99% | 20% |
| C-suite | 3% | 2.00× | 0h / 100% | 0%; immune |
| CEO | 1.5% | 2.50× | 0h / 100% | 0%; immune |
| Board Member | 0.4% | 3.00× | 0h / 100% | 0%; immune |
| VC | 0.1% | Eligible-holder share (§8) | Holding role | No work draw |
| Founder | 2% | 1.00× | Instant; self-employed | No firing |
...
### 3.2 Hiring state machine

Intern–Director: Unemployed → Application Warm-up → Hiring Result → Employed → Work. Failure returns to Unemployed; firing or resignation adds reapplication delay. Hiring repeats after birth or job loss, not every quest. Jobs survive completed quests and ordinary NFT custody transfers. Rewrapping creates a new employment record; callbacks must match its generation.

1. Browse and preview compatible jobs, odds, workload, stress, weight and costs. Current owner or authorized fixed quest beneficiary applies while unemployed, outside recovery, crash, burnout or reapplication blocks, with no conflicting active or queued work. One random application per generation per Payroll day; one unresolved application. No protocol application fee. Disclose and separately fund network and randomness costs.
2. Group 60-second intakes by warm-up. Freeze posting version, title, ID / generation, nonce, odds and readyAt = batchClose + tier warm-up. Request JOB_APPLICATION randomness only at readyAt; intake and provider delays are additional. Success binds job and employer; failure permits a later-day attempt. No seed retry. Transfers preserve pending results and limits; retired callbacks use retired bookkeeping only.
3. C-suite, CEO and Board use deterministic compatible standing appointments, subject to existing health and custody restrictions; no warm-up or hiring roll. Founder and VC use their standing roles.
4. Resignation requires no active or queued work or unapplied performance. Clear job; below C-suite apply the firing delay (§5). Protected executives may reclaim an appointment without added delay. Re-employment preserves earned Job Title and career progress. Firing and resignation preserve Birth Title and earned Job Title but reset careerStreak and promotionProgress; health, XP, history and calendar limits remain. JUICE clears cooldowns, not hiring warm-up or job loss; §9 hiring services provide that bypass.

### 3.3 Career progression — v1 balance
...
**Credit:** One qualifying completed, originally funded daily work entry adds 1 XP, 1 careerStreak and 1 promotionProgress, once. The character must still be employed after that entry’s ordered discipline result. A mistake or zero Payroll result still counts if it does not cause firing. An 8h, 16h or 24h entry earns one credit; 48h earns two across two dates, never extra credit per hour or CRCL earned. Founder earns XP and a venture streak on completed entries, including flops, but never employee promotion progress. VC holding earns no work XP or progression. Unaccepted work earns no credit. Paid-notice entries after firing earn no career or promotion credit; existing XP rules for completed notice entries remain. Custody-fault notice entries mature normally under §4.3.
...
Once hired and eligible, the player can request Clock In at any time. Preview the next start, budget, work end, custody wait, dates, risk and remaining blockers before signing. If that next start lacks funding, show it as unavailable and leave the NFT unlocked; do not silently defer or create an indefinite waiting lock. A later hour requires a fresh quote when it becomes the next start; the explicit queued-successor exception is in §4.3. Existing hiring warm-up, one entry per ID/date, custody, recovery and pending-result limits remain. Hourly starts do not create hourly XP or extra daily pay entitlements.

### 4.2 Duration, entry dates and custody

QuestEscrow holds the existing ACTIVE ERC-721 and preserves its character. It is not HybridVault inventory. Clock In, maturity and withdrawal never unwrap, rewrap or redraw the NFT. Custody starts only in the atomic successful acceptance transaction, after all required budgets and entries are reserved. Work starts at the quoted grid timestamp; the preceding wait is less than one hour for a normal next-start quote, not additional paid work.
...
At 80, latch burnout: block new work or ordinary applications until complete off days reduce stress below 60 or JUICE clears it. Starting at 80 needs two days to 40; 100 needs three. Example: full day at 60 reaches 80; weekend full work adds 30 versus 20, or 24 versus 16 with spouse. Health and calendar restrictions still apply. Overlapping burnout, recovery and crash requirements do not add duplicate off days.
...
JUICE cannot bypass locks, daily entry / random-application limits, pending-result caps, funding, settlement waits, pauses or hiring warm-up; cannot restore a job, erase old mistakes or alter claims. Dedicated hiring services handle warm-up (§9). Validate repeat-dose participation / cost against the fixed Payroll budget; no added emissions.

### 5.3 Mistakes, discipline and career settlement

One verified cohort seed provides independent WORK_PERFORMANCE and DISCIPLINE domains per cohort / ID / generation / paid date; no second provider request. Freeze risk, title, duration and strike odds before requesting the seed. Mistake halves weight and resets clean streak; an employed Intern–Director gains a strike only when the tier discipline draw succeeds. Protected executives can lose weight but never gain strikes or get fired. Founder has no employer discipline; VC has no performance draw.

Three consecutive clean entries clear one strike and reset clean streak. Two strikes fire an Intern–Director: clear job, strike and clean counters, increment Times Fired, preserve earned Job Title, stress and XP, and reset careerStreak and promotionProgress. Accepted work remains paid notice with committed outcomes and XP; do not fire the already-unemployed character again. Stop new Clock In once firing applies. **No Pay never adds a strike or directly fires a worker.** Only the independent, frozen mistake/discipline result can do that; both events can coincide. C-suite, CEO, Board and Founder retain firing immunity. This separates ordinary payout variance from career loss while retaining the health-driven risk of being fired.

Earliest reapply date = max(notificationDay, lastCommittedWorkDay) + 2: one complete uncommitted day after the later date. Other health blocks may delay further. JUICE clears delay and health restrictions, not accepted work, pending results, job loss or used application attempts. Resignation below C-suite uses this delay.
...
Commit one employee and one Founder 10,000-slot outcome book per funding group before any Clock In. Reset only for a new funded group, never a retry. Birth track selects the book; accepted current Job Title supplies pay and discipline factors. Sample without replacement in canonical accepted-entry order using lazy Fisher–Yates, independent EMPLOYEE_LABEL / FOUNDER_LABEL domains and unbiased rejection sampling. No draw occurs before the whole group's final start closes. Duration and mistakes change weight, not label odds. One draw per permanent ID/paid date; a 48 shift draws once in each paid-date group. Partial books need not produce a jackpot.
...
These are fixed v1 opening book probabilities, not independent with-replacement draws or promised dollar returns. Employee positive-label chance is 80%, with Bonus or Jackpot on 20% of draws; Founder has 30% positive labels and twice the employee jackpot probability, offset by many more flops. Neither track is promised a higher expected income. No Pay has no extra discipline effect (§5.3). Minimum Wage is a variable game label; zero after rounding is displayed as 0 CRCL, not a paid wage. VC has no lottery.

Both books share the same group's budget. Let B_w = cohortBudget; w_i = effective title/duration/performance weight; W = sum of positive-label weights; q_i = w_i for Bonus, 25 × w_i for Jackpot, otherwise 0; Q = sum(q_i). Allocate 60% to ordinary pay and 40% to larger wins:
...
UI states: Work Scheduled, Working, Result Set / Locked, Result Pending, No Pay, Available to Claim, Claimed. Show exact start, work end, result readiness and claim status separately. Show gross and claimable CRCL, with zero protocol claim fee; network costs remain separate. Minimum Wage, Bonus and Jackpot are variable results. Quotes show actual funded budget, current/max group weights, odds, four-start group allocation and ranges, not a fixed stock return. A different funding group’s attendance can change the eventual share even with the same title and duration. Show payout percentiles from settled groups, including zero results, sample size and period; never show winners-only averages, fixed APY, guaranteed minimum income or theoretical maximum as typical pay.
...
An unemployed Intern–Director with no active or queued work may buy one compatible job: skip the warm-up and hiring roll, clear reapply delay and bind job immediately. Preserve title, health, custody and Payroll limits. Degree or referral badge may remain cosmetic history; hire benefit is consumed. Exclude already-employed, Founder and automatically appointed executives or VCs.

Pending application before seed request: retire atomically as BYPASSED. After request: wait for bound result. A failed random attempt that day does not block deterministic paid or sponsored hire or permit another random attempt. Failed appointment reverts burn and state. Tired hires still need health eligibility, funded Clock In and completed work to earn.
...
- **Career and health:** Employer or role, Stress and Mood, strikes, XP, careerStreak, promotionProgress, next title/threshold, progressionConfigVersion, Times Fired and pending performance count.
...
- **Job Board:** Compatible jobs; hiring warm-up and odds, distinct from rarity odds; firing protection and strike chance; paid or sponsored hiring; reapplication date and earliest funded work.
...
- **Conversion:** Wrap / capture and unwrap / release quotes from §2; balances, current inventory, permanent reset loss, retained claims, 2-USDC entry fee and zero exit fee. Show request, randomness, assignment and delivery separately; retries retain the assignment.
- **Work desk:** Timing from §4, funded budget, weights, zero-pay probability and estimated award ranges.
...
- **EmploymentRegistry / JobBoard:** Eligibility, jobs, applications and ordered career results.
...
- **ProgressionConfig:** Immutable employee ladder, per-rung thresholds and complete employer/title appointment mappings, versioned and committed at birth. No Founder or VC transition.
- **EmploymentState:** Character, current Job Title, Birth Title, job, employer; careerStreak, promotionProgress, progressionConfigVersion and last credited day; application nonce, day and result; result cursor and count; strikes, clean streak, XP, firings and reapply date.
- **JobPosting / Application:** Tier rules and version; frozen odds, warm-up and readyAt; NFT ID, generation, date and nonce; request, domain and terminal result.
...
- **Entry:** Epoch and cohort, paid date, NFT ID, generation, original shift/end and canonical index; beneficiary and job; title, duration, track, stress, risk, perk/version and strike odds; performance, discipline and effective weight; eligibility, label, gross amount, finalizedAt, claimableAt and paid flag. No advance or protocol claim-fee fields.
...
- **previewConvert(quantity, direction):** Return fixed principal, fee, versions and draw odds; unwrap quotes also bind IDs/generations and show losses and retained claims.
- **requestNFTs(quantity, recipient, maxFee, deadline, configHash):** Request wrap; transfer exact principal + fee and reserve existing inventory capacity; return request IDs.
...
- **redeemNFTs(ordered tokenIds, expectedGenerations, recipient):** Unwrap; checkpoint VC, retire current characters, transfer existing NFTs into inventory and return exact principal atomically.
...
- **listJobs / previewApplication(tokenId):** Return postings, employment state, cooldown, odds and cost.
- **applyForJob(tokenId, expectedGeneration, jobId, postingVersion, deadline):** Commit one application.
- **processApplications(batchId, maxEntries):** Apply bound outcomes once.
...
- **previewShift(tokenId, requestedStartAt, shiftKind):** Return the next valid quoted hourly start, exact segment/full-shift ends, custody wait, health/job eligibility, risk, weights, rest/pass requirements, funded cohort references and expiry. A normal request cannot silently skip an unavailable next start.
- **clockIn(tokenId, expectedGeneration, startAt, shiftKind, beneficiary, configHash, deadline):** Validate the exact quote before cutoff and reserve all cohort entries, date/role markers and custody atomically. Enforce the normal next-start rule or explicit queued-successor exception.
...
- **Employment:** JobApplied, JobApplicationResolved, JobResigned, JobFired, ExperienceGranted, CareerProgressed, CareerStreakReset, JobPromoted, HireBypassed, ReferralCampaignChanged, ExecutiveReferralRedeemed.
...
- **Employment:** NotEmployed, ApplicationPending, ApplicationCooldown, TitleIneligible, EmploymentResultsPending.
